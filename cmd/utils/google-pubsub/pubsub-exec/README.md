...
- `status`: Current execution state or final outcome
- `done`: Present and `true` when command execution has finished, whether it succeeded or failed
- `exitCode`: Command exit code when execution has finished (0 = success, non-zero = error)

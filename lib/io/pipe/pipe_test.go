...
func TestReader_Flush(t *testing.T) {
	...
	t.Run("nil flusher does not error", func(t *testing.T) {
		...
		err := r.Flush()
		assert.NoError(t, err)
		assert.Equal(t, 0, r.Len(), "buffer should be empty after flush")
	})
...
func TestReader_Start_FlusherError(t *testing.T) {
	...
	mockFlusher.AssertExpectations(t)
	assert.Equal(t, 0, r.Len(), "buffer should be cleared even if flush fails")
...
func TestReader_Start_ThresholdDisabled(t *testing.T) {
	...
	mockFlusher.AssertNotCalled(t, "Flush", mock.Anything)
	assert.Greater(t, r.Len(), 0, "buffer should contain data as no auto-flush occurred")
	...
	assert.Equal(t, 0, r.Len(), "buffer should be empty after manual flush")
}
...
func TestReader_ConcurrentFlush(t *testing.T) {
	...
	assert.Equal(t, 0, r.Len(), "buffer should be empty after concurrent flushes")
}

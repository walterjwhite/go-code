...
func (t *TaleoATS) RegisterAccount(executor *atsclient.Executor, account *Account) error {
	...
	err := executor.Navigate(baseURL)
	if err != nil {
		return fmt.Errorf("failed to navigate to Taleo: %w", err)
	}
	...
	for selector, value := range fields {
		if value != "" { // Only fill fields that have values
			...
			if err != nil {
				...
				return fmt.Errorf("failed to fill required field: %w", err)
			}
	...
	log.Println("Taleo account registration completed")
	return nil
}
...
func (t *TaleoATS) LoginAccount(executor *atsclient.Executor, email, password string) error {
	...
	err := executor.Navigate(baseURL)
	if err != nil {
		return fmt.Errorf("failed to navigate to Taleo login page: %w", err)
	}
	...
	log.Println("Taleo login completed")
	return nil
}

func (t *TaleoATS) ApplyForJob(executor *atsclient.Executor, resumePath string, qaMap map[string]string, aiEnabled bool) error {
	log.Println("Starting Taleo job application process")


	if resumePath != "" {
		log.Println("Uploading resume")
		...
		for _, selector := range resumeSelectors {
			err := t.uploadFile(executor, selector, resumePath)
			if err == nil {
	...
	for questionPattern, answer := range qaMap {
		...
		if err != nil {
			...
			if err != nil {
				log.Printf("Could not find text field for question pattern '%s'", questionPattern)
			} else {
				log.Printf("Filled text field for question pattern '%s' with answer '%s'", questionPattern, answer)
			}
		} else {
			log.Printf("Filled text field for question pattern '%s' with answer '%s'", questionPattern, answer)
		}
		...
		if err != nil {
			log.Printf("Could not select radio/checkbox for answer '%s': %v", answer, err)
		} else {
			log.Printf("Selected radio/checkbox for answer '%s'", answer)
		}
		...
		if err != nil {
			log.Printf("Could not select dropdown option for question pattern '%s': %v", questionPattern, err)
		} else {
			log.Printf("Selected dropdown option for question pattern '%s' with answer '%s'", questionPattern, answer)
		}
	}
	...
	log.Println("Taleo job application completed")
	return nil
}

func (t *TaleoATS) uploadFile(executor *atsclient.Executor, selector, filePath string) error {
	return executor.SetValue(selector, filePath)
}

func (t *TaleoATS) selectDropdownOption(executor *atsclient.Executor, selector, optionValue string) error {

...
type FileMatcher interface {
	MatchesDirectory(root, filePath string, f os.DirEntry) bool
	Matches(root, filePath string, f os.DirEntry) bool
...
func (m *FilenamePatternMatcher) Matches(root, filePath string, f os.DirEntry) bool {
	return matchesPattern(filePath, m.Patterns...)
}

func (m *FilenamePatternMatcher) MatchesDirectory(root, filePath string, f os.DirEntry) bool {
	return true
}
...
func (e *HiddenFileExcluder) Matches(root, filePath string, f os.DirEntry) bool {
	return strings.Index(f.Name(), ".") != 0
}

func (e *HiddenFileExcluder) MatchesDirectory(root, filePath string, f os.DirEntry) bool {
	return strings.Index(f.Name(), ".") != 0
}
